...
* `window.alert();` : 윈도우 위 경고메세지  표시
* `alert();` : : 윈도우 위 경고메세지  표시
* `value` : 값
* `length` : 길이
### DOM(Document Object Model)
* `documnet` 본문 최상위 객체 대상으로 속성, 메소드 이용, 생략할 수 없다.
...
------
* `typeof` : input type 확인  
* `string` : 문자
* `number` : 숫자 
* 문자일 경우 앞에 `Number` 를 써주면 문자에서 숫자로 바뀐다.
------
...
----
* `childNodes`, `children`
* `header>nav`관계 DOM이 있을 때 `header`의 자식을 선택할 때 이용할 수 있다.
* `DOM.cildNodes`, `DOM.children`
----
* `firstChild`, `firstElementChild`
* 첫번째 자식, 첫 번째 자식 요소 노드
* `lastChild`, `lastElementChild`
* 마지막 자식, 마지막 자식 요소 노드
* `nextSibling`, `nextElementSibling`
* 공백포함 다음 형제, 다음 형제 요소만 인식 속성
* `previousSibling`, `previousElementSibling`
* 공백포함 이전 형제 속성, 이전 형제 요소만 인식 속성
----
## 자바스크립트 객체 속성
* `innerHTML` : 객체.속성
