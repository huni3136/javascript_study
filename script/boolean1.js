...
if(b!=a){console.log("b는 참이 아니다")} //o
console.log('===========')
...
login_btn.addEventListener('click',()=>{
    ...
    if(user_id.value == 'huni3136'){
        // == > < != >= <=
        //input의 값이 빈문자열이 아닐 때 참
        alert(`관리자 ${user_id.value}님 어서오세요`)
    }else{
        alert(`일반 회원 ${user_id.value}님 어서오세요`)
    }
})
console.log('--------------')
//사용자가 입력한 숫자가 짝수일경우만 (홀수는 반응없음)
//팝업으로 "당신이 입력한 ?? 수는 짝수입니다." 출력
// 짝수구분 나머지 연산자로 2를 나눈 나머지 값 체크하기 
const user_number = document.querySelector('#user_number')
const user_btn = document.querySelector('#number_btn')
user_btn.addEventListener('click',()=>{
    if(user_number.value % 2){
        alert(`당신이 입력한 ${user_number.value}수는 홀수입니다.`)
    }else{
        alert(`당신이 입력한 ${user_number.value}수는 짝수입니다.`)
    }

})
console.log('--------------')
//5천보 이상일 경우 "목표 달성 완료!!" 출력
const user_walk = document.querySelector('#walk')
const walk_btn = document.querySelector('#walk_btn')
console.log(user_walk,walk_btn)
walk_btn.addEventListener('click',()=>{
    if(user_walk.value >= 5000){
        alert(`${user_walk.value }보 목표 달성 완료!!`)
    }else{
        alert(`목표 달성 실패 ㅠㅠ ${5000 - user_walk.value}보 부족합니다!!`)
    }
})
//----------------
