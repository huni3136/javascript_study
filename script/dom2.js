...
console.log(header)
//tag, classs 등과 같이 다수가 존재할 수 있는 대상을 DOM으로 선택하면 특정 대상 1개만 잡히는게 아닌 COllection기준으로 여러개가 잡히기 때문에 원하는 대상 사용 시 정확히 그 대상을 인덱스로 선택해야 한다.
...
*/
/* 
변수 선언 시 인덱스를 작성하고 선언했다면 변수 사용 시 인덱스를 붙이지 않고 변수명만 간편하게 쓸 수 있다. 대신, 인덱스가 고정되므로 다른 인덱스번호에 해당하는
태그나 클래스를 사용하고 싶을때 변수를 새로 만들어야 한다.
console.log(aTag2);
...
function btn_func4(){
    ...
    //제품 1개당 1000원
    //"요청사항은 000, 주문수량은 0개, 총 결제금액은 000원 입니다."
    let total = Number (input_num.value);
    let total2 = input_text.value;
    let price = 1000
    alert(`요청사항은 ${total2}, 주문수량은 ${total}개, 총 결제금액은 ${total * price}원 입니다.`);
}
//객체.메서드(실행값);
//객체.속성 = 속성에 대입할 값;